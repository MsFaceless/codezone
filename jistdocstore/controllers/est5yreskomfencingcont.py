...
__all__ = ['Estimating_5yr_Eskom_Fencing_Controller']


class Estimating_5yr_Eskom_Fencing_Controller(BaseController):
    ...
    @expose()
    def index(self):
        redirect('est5yreskomfencingcont/eskom_5yr_fencing_console')
    ...
    @expose('jistdocstore.templates.estimating.eskom_5yr_fencing_console')
    def eskom_5yr_fencing_console(self,**named):
        ...
        sites = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
               order_by(asc(JistEstimating5yrEskomFencingSites.id)).all()
        boq = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingItems). \
               order_by(asc(JistEstimating5yrEskomFencingItems.id)).all()
        return dict(page='Eskom 5yr Fencing Estimate Console',
                    contracts = contracts,
                    users = activeusers,
                    boq = boq,
                    myjistid = myid,
    ...
    def get_estimate_sites(self,**kw):
        ...
        if kw['switch']=="All":
            sites = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
                   order_by(desc(JistEstimating5yrEskomFencingSites.id)).all()
            ...
            sites = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
                    filter(JistEstimating5yrEskomFencingSites.name.like(searchphrase)). \
                   order_by(desc(JistEstimating5yrEskomFencingSites.id)).all()
            ...
            searchphrase = "%"+sitename+"%"
            sites = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
                    filter(JistEstimating5yrEskomFencingSites.description.like(searchphrase)). \
                   order_by(desc(JistEstimating5yrEskomFencingSites.id)).all()
    ...
    def save_new_estimate_sites(self,**kw):
        ...
        new_site = JistEstimating5yrEskomFencingSites(name = name,
        ...
        DBS_Jist5yrEskomFencing.add(new_site)
        DBS_Jist5yrEskomFencing.flush()
        return

    @expose()
    def get_edit_estimate_sites(self,**kw):
        site = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
                filter(JistEstimating5yrEskomFencingSites.id == kw['siteid']). \
                one()
        html = """
                        ...
                        <fieldset>
                            ...
                            <br/>
                            <button class="ui-widget ui-widget-content ui-state-default" id="button_do_edit_new_site">Edit Site</button> 
                            <button class="ui-widget ui-widget-content ui-state-default" id="button_do_cancel_edit_site">Cancel</button>
                        </fieldset>
                        </form>
    ...
    def save_edit_estimate_sites(self,**kw):
        site = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingSites). \
                filter(JistEstimating5yrEskomFencingSites.id == kw['edit_siteid']). \
                one()
    ...
    def get_estimate_quote_list(self,**kw):
        quotelist = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuotes). \
                filter(JistEstimating5yrEskomFencingQuotes.idsite == kw['siteid']). \
                order_by(desc(JistEstimating5yrEskomFencingQuotes.id)).all()
        html = """
               <table id="quote_list">
               <th>Quote No </th>
               <th>Quote Date</th>
    ...
    def get_estimate_quote_items(self,**kw):
        quoteitems = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.idquote == kw['quoteid']). \
                all()
        quoteitemsvalue = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.idquote == kw['quoteid']). \
                value(func.sum(JistEstimating5yrEskomFencingQuoteBQItems.total))

        html = """
               <h3 class="ui-widget-shadow">Quote Number: %s 
               <span style="float:right">%s</span>
                    <a href="/est5yreskomfencingcont/ajaxexport_excelquote/%s">
    ...
    def get_estimate_quote_items_edit(self,**kw):
        quoteitem = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.id == kw['itemid']). \
                one()
        html = """
    ...
    def ajaxaddnewquotefull(self,jcno, **kw):
        ...
        usernow = User.by_user_name(username)
        useridcreated = usernow.user_id
        new_quote = JistEstimating5yrEskomFencingQuotes(
                                       idsite = jcno,
        ...
        DBS_Jist5yrEskomFencing.add(new_quote)
        DBS_Jist5yrEskomFencing.flush()
        scp_items = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingItems). \
                    all()
        #print new_quote
        for item in scp_items:
            new_quote_bqitem = JistEstimating5yrEskomFencingQuoteBQItems(
            ...
            DBS_Jist5yrEskomFencing.add(new_quote_bqitem)
        DBS_Jist5yrEskomFencing.flush()
    ...
    def ajaxdeletequoteitem(self, **kw):
        ...
        quoteitem = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.id == kw['itemid']). \
                one()
        DBS_Jist5yrEskomFencing.delete(quoteitem)
        return
    ...
    def ajaxaddnewquote_oneitem(self,quoteid,bqitemid, **kw):
        ...
        usernow = User.by_user_name(username)
        useridcreated = usernow.user_id
        item = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingItems). \
                filter(JistEstimating5yrEskomFencingItems.id == int(bqitemid)). \
                    one()
        new_quote_bqitem = JistEstimating5yrEskomFencingQuoteBQItems(
                                       idquote = quoteid,
                                       idbqitem = item.id,
    ...
    def ajaxexport_excelquote(self,quoteid, **kw):
        quoteitems = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.idquote == quoteid). \
                all()
        quoteitemsvalue = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingQuoteBQItems). \
                filter(JistEstimating5yrEskomFencingQuoteBQItems.idquote == quoteid). \
                value(func.sum(JistEstimating5yrEskomFencingQuoteBQItems.total))
        ...
        #print filename
        templatename = "EskomTemplate.xls"
    ...
    def estuploadfile(self,*arg,**kw):
        ...
        usersharetable = []
        new_file = JistEstimating5yrEskomFencingPhotos(filename=big_file_base,
        ...
        #    print col
        #return
        DBS_Jist5yrEskomFencing.add(new_file)
        DBS_Jist5yrEskomFencing.flush()
    ...
    def get_photos_by_site(self,siteid,**kw):
        ...
        htmlheader = """
                    ...
                    """%(siteid)
        htmlout = ''
        thisuploads = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingPhotos). \
                filter(JistEstimating5yrEskomFencingPhotos.jcno==int(siteid)). \
                all()
    ...
    def estimating_pic_viewer(self,**kw):
        ...
        thisupload = DBS_Jist5yrEskomFencing.query(JistEstimating5yrEskomFencingPhotos). \
                filter(JistEstimating5yrEskomFencingPhotos.filename==fname). \
                one()
        username = request.identity['repoze.who.userid']
        usernow = User.by_user_name(username)
        ...
        return dict(page='JIST Estimating: Eskom Picture Viewer',
                   appname = 'Eskom5yrFencing',
                   srcpic=srcpic,
