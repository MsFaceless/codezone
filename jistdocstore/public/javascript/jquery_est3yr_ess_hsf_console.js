$(document).ready(function() {
 ...
 //           console.log("beforeActivate pressed old tab is:" +ui.oldPanel.label) 
  ...
  //          console.log("beforeActivate pressed new tab is:" +ui.panel) 
    ...
    $("#button_view_all_sites_hsf").button();
    $("#button_search_site_description_hsf").button();
    $("#button_search_site_name_hsf").button();
    $("#button_add_new_est_site_hsf").button();
    ...
    $("#button_view_all_sites_hsf").click(function(){
        $("#div_est_sites").load("/est3yresshsfcont/get_estimate_sites?switch=All",function(responseTxt,statusTxt,xhr){
            activate_table_sites();
        });
    });
    $("#button_search_site_name_hsf").click(function(){
        var searchphrase = $("#search_sitename_hsf").val()
        $("#div_est_sites").load("/est3yresshsfcont/get_estimate_sites?switch=SearchName&searchphrase="+alltrim(searchphrase),function(responseTxt,statusTxt,xhr){
            activate_table_sites();
        });
    });
    $("#button_search_site_description_hsf").click(function(){
        var searchphrase = $("#search_sitedescription_hsf").val()
        $("#div_est_sites").load("/est3yresshsfcont/get_estimate_sites?switch=SearchDescription&searchphrase="+alltrim(searchphrase),function(responseTxt,statusTxt,xhr){
            activate_table_sites();
        });
    });
    $("#button_add_new_est_site_hsf").click(function(){
        $("#estimate_newsite").show();
    });
    $("#button_do_add_new_site_hsf").click(function(){
        var formserial = $("#frm_est_new_site").serialize();
        var jqxhr = $.post("/est3yresshsfcont/save_new_estimate_sites?"+formserial, function(data) {
            clear_form_elements($("#frm_est_new_site"));
            $("#estimate_newsite").hide();
            $("#button_view_all_sites_hsf").trigger("click");
            return false;
        });
            return false;
    });
    $("#button_do_cancel_new_site_hsf").click(function(){
        clear_form_elements($("#frm_est_new_site"));
        $("#estimate_newsite").hide();
        return false;
    });
        ...
        $("#estimate_sites_table").delegate('td','click',function(e){
            if ($(this).parent().index() != 0){ 
                ...
                if ($(this).index() == 9){ 
                    ...
                    $("#estimate_editsite").load("/est3yresshsfcont/get_edit_estimate_sites?siteid="+alltrim(siteid),function(data){
                        $("#estimate_editsite").show();
                        $("#button_do_cancel_edit_site_hsf").click(function(){
                            clear_form_elements($("#frm_est_new_site"));
                            $("#estimate_editsite").hide();
                            return false;
                        });
                        $("#button_do_edit_new_site_hsf").click(function(){
                            var formserial = $("#frm_edit_est_new_site").serialize();
                            var jqxhr = $.post("/est3yresshsfcont/save_edit_estimate_sites?"+formserial, function(data) {
                ...
                if ($(this).index() == 10){ 
                    $("#estimate_quote_list").load("/est3yresshsfcont/get_estimate_quote_list?siteid="+alltrim(siteid),function(data){
                        ...
                        $("#quote_list").delegate('td','click',function(e) {
                          ...
                          $("#activequoteidindex").val(thisquoteindex);
                            $("#estimate_quote_items").load("/est3yresshsfcont/get_estimate_quote_items?quoteid="+alltrim(thisquoteid),function(data){
                                ...
                                $("#quote_items").delegate('td','click',function(e) {
                                    if ($(this).parent().index() != 0){ 
                                        ...
                                        if ($(this).index() == 6){ 
                                            $("#estimate_quote_items_edit").load("/est3yresshsfcont/get_estimate_quote_items_edit?itemid="+alltrim(thisitemid),function(data){
                                                ...
                                                $( "#button_do_edit_quote_item" ).click(function(e){
                                                    ...
                                                    var jqxhr = $.post("/est3yresshsfcont/save_edit_estimate_item?itemid="+alltrim(thisitemid)+"&"+formserial, function(data) {
                                        ...
                                        if ($(this).index() == 7){ 
                                            ...
                                            var jqxhr = $.post("/est3yresshsfcont/ajaxdeletequoteitem?itemid="+alltrim(thisitemid),function(responseTxt,statusTxt,xhr){
                                                            $("#quote_list").find("tr").eq(thisquoteindex).find("td").trigger("click");
                                            });
                    ...
                    $("#ess_3yr_hsf_fencing_tabs").tabs('select', 2);
                    $("#hsf_pics").load("/est3yresshsfcont/get_photos_by_site?siteid="+alltrim(siteid),function(data){
                        $('.thumb_clicked').click(function() {
                            //console.log($(this));
                            var picpath = $(this).attr('value');
                            window.open('/est3yresshsfcont/estimating_pic_viewer?fname='+picpath, '_blank');
                            return false;
                        });
                    });
                };
    ...
    $("#ess_3yr_hsf_fencing_tabs").tabs('select', 1);
    $("#button_do_add_new_quote_hsf").click(function(){
        var siteid = $("#activesiteid").val();
        var siteidindex = $("#activesiteidindex").val();
        var jqxhr = $.post("/est3yresshsfcont/ajaxaddnewquotefull/"+alltrim(siteid),function(responseTxt,statusTxt,xhr){
            $("#estimate_quote_list").load("/est3yresshsfcont/get_estimate_quote_list?siteid="+alltrim(siteid),function(data){
    ...
    $("#button_do_add_new_quote_hsf_empty").click(function(){
        var siteid = $("#activesiteid").val();
        var siteidindex = $("#activesiteidindex").val();
        var jqxhr = $.post("/est3yresshsfcont/ajaxaddnewquoteempty/"+alltrim(siteid),function(responseTxt,statusTxt,xhr){
            $("#estimate_quote_list").load("/est3yresshsfcont/get_estimate_quote_list?siteid="+alltrim(siteid),function(data){
                $("#estimate_sites_table").find("tr").eq(siteidindex).find("td").eq(10).trigger("click");
                    activate_table_sites;
                });
    ...
    $("#boqlist_hsf").delegate('tr','mouseover mouseleave click',function(e) {
        e.preventDefault();
        if (e.type == 'mouseover') {
            $(this).addClass("hover");
        } else if ( e.type == 'click' ) {
    ...
    $("#boqlist_hsf").delegate('td','click',function(e){
        if ($(this).parent().index() != 0){ 
            if ($(this).index() == 5){ 
                if ($("#activequoteid").val()){
                    ...
                    var jqxhr = $.post("/est3yresshsfcont/ajaxaddnewquote_oneitem/"+alltrim(quoteid)+"/"+alltrim(bqitemid),function(responseTxt,statusTxt,xhr){
                            //$("#estimate_sites_table").find("tr").eq(siteidindex).find("td").eq(10).trigger("click");
                            $("#ess_3yr_hsf_fencing_tabs").tabs('select', 2);
    ...
    var jqueryuploader_hsf = $('#jquery-fine-uploader_hsf').fineUploader({
        request: {
                     endpoint: '/est3yresshsfcont/estuploadfile'
                 },
        autoUpload: false,
        validation: {
        ...
        $("#hsf_pics") .load("/est3yresshsfcont/get_photos_by_site?siteid="+alltrim(activesiteid),function(data){
        });
        //console.log(response); 
    });
    $('#triggerUpload').click(function() {
        jqueryuploader_hsf.fineUploader('uploadStoredFiles');
