...
###############################################################################
    ...
    def edit_product(self, *args, **kwargs):
        html = self.product_new(*args, **kwargs)
        javascript = self.get_javascript_new_product_onload()
        title = f"""Save or Edit Product"""
        return dict(title=title, html=html, javascript=javascript)

    @expose('rocket.templates.generic')
    def product_new(self, *args, **kwargs):

        #card_header = self.get_edit_product_card_title_html(**kwargs)
        # tab 1
        tab1_content = self.get_product_details_html(**kwargs)
        # tab 2
        tab2_content = self.get_edit_product_benefit_details_html(**kwargs)
        #tab 3
        tab3_content = self.get_edit_product_message_html(**kwargs)
        # tab 4
        tab4_content = self.get_edit_product_documents_html(**kwargs)
        # tab 5
        tab5_content = self.get_edit_product_summary_html(**kwargs)
    ...
    def get_product_details_html(self, *args, **kwargs):
        ...
        dropdown_product_type = self.get_selectbox_product_types()
        dropdown_product_insurer_type= self.get_selectbox_product_owner()
        html= f"""
	<div class="row">
		<div class="col-md-12">
			<div class="card">
            ...
            <form id="form_new_product">
						...
						<div class="col-md-6">
							<div class="form-group row">
								...
								<div class="col-md-8">
									<input id="code" name="code" type="text"  class="form-control" required='true'>
								</div>
							</div>
                            <div class="form-group row">
								<label class="col-md-4 col-form-label">{_('Policy Number Prefix')}</label>
								<div class="col-md-8">
									<input id="policy_number_prefix" name="policy_number_prefix" type="text"  class="form-control" required='true'>
								</div>
							</div>	
							</di>
						</div>
						   ...
						   <div class= "row">
                             ...
                             <div class="col-md-6 text-right">
									<button class="btn btn-outline-primary entity_back">Save & Continue</button>
								</div>
							</div>
				</div>
			</div>
        ...
        <script>
            ...
            var valid = FormIsValid("#form_new_product");
             if(valid){
                ...
                $.post('/product/save_new_or_edit_product?', formserial, function(data){
                    ...
                    console.log(kwargs);
                     $('.nav-link:eq(1)').trigger('click');
                             $('#tab2').load('/product/get_edit_product_benefit_details_html?', kwargs, function(data){
                              return false;
                      });
                    };
                    return false;
    ...
    @expose()
    def save_new_or_edit_product(self, *args, **kwargs):

        product_state_type_id = TYPEUTIL.get_id_of_name('product_state_type', 'sandbox')
        dict_product = {
            'entity_org_product_owner_id': kwargs.get('entity_org_product_owner_id'),
            'code': kwargs.get('code'),
            'name': kwargs.get('name'),
            'product_type_id': kwargs.get('product_type_id'),
            'product_state_type_id': product_state_type_id,
            'policy_number_prefix': kwargs.get('policy_number_prefix'),
        }


        product_id=self.save_new_productobj(**dict_product)
        dict_benefit ={
            "product_id": product_id,
        }
        self.save_new_benefitobj(**dict_benefit)

        return json.dumps({'success': True, 'product_id': product_id})


    def save_new_productobj(self, *args, **kwargs):

        usernow = request.identity['user']
        this = Product()
        this.product_type_id = kwargs.get('product_type_id', None)
        this.code = kwargs.get('code', None)
        this.name = kwargs.get('name', None)
        this.entity_org_product_owner_id = kwargs.get('entity_org_product_owner_id', None)
        this.policy_number_prefix = kwargs.get('policy_number_prefix', None)
        this.product_state_type_id = kwargs.get('product_state_type_id', None)
        this.added_by = usernow.id
        DBSession.add(this)
        DBSession.flush()
        return  this.id

    def save_new_benefitobj(self, *args, **kwargs):
        usernow = request.identity['user']
        this = Benefit()
    ...
    @expose()
    def get_edit_product_benefit_details_html(self, *args, **kwargs):
        benefit_table = self.get_benefits_html_table(**kwargs)
        html= f"""
	<div class="row">
		<div class="card">
			<div class="card-header">
   ...
   <div class="row d-flex">
                        <div class="col-md-6">
                            <h4 class="card-title">{_('Benefits Details')}</h4>
                        </div>
                        <div class="col-md-6 text-right">
                            <button id="create_new_benefit" class="btn btn-primary ml-auto">New Benefit</button>
                        </div>
                    </div>
				<hr>
			</div>
				...
				<div class="row">
					...
					<div class="col-md-7">
						<div id="accordion" role="tablist" aria-multiselectable="true" class="card-collapse">
								...
								<div class="card-header" role="tab" id="benefit_details">
								...
								<div id="collapseOne" class="collapse show" role="tabpanel" aria-labelledby="headingOne">
									<div class="card-body">
										<div id="div_benefit_details"></div>
									</div>
								</div>
							</div>
							<div class="card card-plain">
								<div class="card-header" role="tab" id="assured">
								...
								<div id="collapseTwo" class="collapse show" role="tabpanel" aria-labelledby="headingOne">
									<div class="card-body">
										<div id="div_assured_details"></div>
									</div>
								</div>
							</div>
							<div class="card card-plain">
								<div class="card-header" role="tab" id="cover_and_exclusion">
								...
								<div id="collapseFour" class="collapse" role="tabpanel" aria-labelledby="headingTwo">
									<div class="card-body">
											<div id="div_cover_details"></div>
									</div>
								</div>
							</div>
							<div class="card card-plain">
								<div class="card-header" role="tab" id="premiums">
								...
								<div id="collapseFive" class="collapse" role="tabpanel" aria-labelledby="headingThree">
									<div class="card-body">
											<div id="div_premium_details"></div>
									</div>
								</div>
							</div>
	<div class="card card-plain">
								...
								<div id="collapseSix" class="collapse" role="tabpanel" aria-labelledby="headingThree">
									<div class="card-body">
											<div id="div_allocation_details"></div>
									</div>
								</div>
							</div>
	<div class="card card-plain">
								...
								<div id="collapseSeven" class="collapse" role="tabpanel" aria-labelledby="headingThree">
									<div class="card-body">
											<div id="div_claim_questions_details"></div>
									</div>
								</div>
							</div>
	<div class="card card-plain">
        ...
        javascript="""
            <script>               
          $(".open_benefit_details").click(function(){
                var kwargs = 'benefit_id='+$(this).attr('benefit_id');
    
                $('#div_benefit_details').load('/product/get_benefit_details_html?', kwargs, function(data){
                    return false;
                });
                
                  $('#div_assured_details').load('/product/get_assured_details_html?', kwargs, function(data){
                    return false;
                });
                
              
            });
            $(".open_benefit_details:eq(0)").trigger('click');
            </script>
        """
        return  html + javascript
    ...
    def get_benefits_html_table(self, *args, **kwargs):
        ...
        #dbase_query = Benefit.by_attr_all('product_id',product_id)
        dbase_query = Benefit.get_all('name')
        outputlist = []
        for item in dbase_query:

            outputlist.append({
                'name': f"<div class='edit edit_contact'  contact_id='{item.id}'>{item.name}</div>",
                'benefit_cover': item.name,
                'benefit_asset_type':  item.name,
                'is_main_benefit':  img_active if item.is_main_benefit else img_inactive,
                'open': f"<div class='open_benefit_details' product_id='{product_id}' benefit_id='{item.id}'>open</div>",
            })
        dbcolumnlist = [
            'name',
            'benefit_cover',
    ...
    @expose()
    def get_assured_details_html(self, *args, **kwargs):
        benefit_id= kwargs.get('benefit_id')
        benefit = Benefit.by_id(benefit_id)
        benefit_assured_life= BenefitAssuredLife.by_attr_first('benefit_id', benefit_id)
       # if not benefit_assured_life: return ''

        dropdown_relationship_type= self.get_selectbox_benefit_assured_life_relationship()
        dropdown_benefit_asset_type = self.get_selectbox_benefit_asset_type()
     #   has_maximum_age = 'checked' if benefit_assured_life.has_maximum_age else ''
        non_life_fields = ''

        if  not benefit.is_life:
            non_life_fields = f"""
                               <div class="form-group row">
								        <label class="col-md-4 col-form-label" required>{_('Asset Type')}</label>
								            <div class="col-md-8">
									          {dropdown_benefit_asset_type}
								        </div>
							        </div>   
							         <div class="form-group row">
								        <label class="col-md-4 col-form-label" required>{_('Description')}</label>
								            <div class="col-md-8">
									             <textarea required="true" name='text' type="text" class="form-control" rows="3" maxlength='250'></textarea>
								        </div>
							        </div>
        ...
        html = f"""  
         <div class="row"> <form id="form_benefit_details"
        						<div class="col-md-12">
        							
        	                        <div class="form-group row">
        								<label class="col-md-4 col-form-label" required>{_('Relationship Type')}</label>
        								<div class="col-md-8">
        									{dropdown_relationship_type}
        								</div>
        							</div>
                                    <div class="form-group row">
								        <label class="col-md-4 col-form-label" required>{_('Maximum Lives Covered')}</label>
								            <div class="col-md-8">
        ...
        <div class="form-group row">
								...
								<div class="col-md-8">
									<input id="maximum_entry_age" type="text" name="maximum_entry_age" class="form-control" required='true'>
								</div>
							</div>

		                           <div class="form-check" style=" padding-bottom: 15px;">
									<label class="form-check-label">
										<input class="form-check-input user_role" name="has_maximum_age" id="has_maximum_age" type="checkbox">
										<span class="form-check-sign">Has  Expiry Age</span>
									</label>
									<div class="row" >
										...
										<div class="col-md-8">
											<div id="expiry_fields" style="display: none;">
												<div class="form-group row">
													<label class="col-md-4 col-form-label" for="">Expiry Age</label>
													<div class="col-md-8">
														<input id="exipry_age" type="number" name="exipry_age" class="form-control">
													</div>
												</div>
											</div>
										</div>
                                ...
                                <div class="form-group row">
                                      ...
                                      <div class="form-group row">
								        <label class="col-md-4 col-form-label" required>{_('Maximum Age')}</label>
								            <div class="col-md-8">
									            <input id="maxmum_age" type="number" name="maxmum_age" class="form-control" required='true'>
								        </div>
							        </div>
	                              {non_life_fields}
        								...
        								<div class="col-md-6">
        									<button id='save_assured_details' class="btn btn-primary">Save</button>
        								</div>
                                     <div class="col-md-6 text-right">
        									<button class="btn btn-outline-primary entity_back">Save & Continue</button>
        								</div>
        ...
        createClickHideShow('#has_maximum_age', '#expiry_fields')
    ...
    @expose('rocket.templates.generic')
    def get_benefit_details_html(self,*args, **kwargs):
        benefit_id= kwargs.get('benefit_id')
        benefit = Benefit.by_id(benefit_id)
        if not benefit: return ''
        is_life = 'checked' if benefit.is_life else ''
        is_compulsory = 'checked' if benefit.is_compulsory else ''
        dropdown_insurer = self.get_selectbox_insurer(benefit.entity_org_insurer_id)
        dropdown_benefit_purchase_type = self.get_selectbox_purchase(benefit.benefit_purchase_type_id)
        html= f"""    <form id="form_benefit_details">
		<input  type="hidden" id="benefit_id" name="benefit_id" class="form-control" value="{benefit_id}"/>
		<div class="col-md-12">
			<div class="form-group row">
				<label class="col-md-4 col-form-label" required>{_('Name')}</label>
				<div class="col-md-8">
					<input id="name" type="text" name="name" class="form-control" required='true' value='{benefit.name}'>
				</div>
			</div>
			<div class="form-group row">
				<label class="col-md-4 col-form-label" required>{_('Purchase Type')}</label>
				<div class="col-md-8">
					{dropdown_benefit_purchase_type}
				</div>
			</div>
			<div class="form-group row">
			...
			<div class="form-group row">
				<label class="col-4 col-form-label" for="active" required>Is Life</label>
				<div class="col-8">
			...
			<div class="form-group row">
				<label class="col-4 col-form-label" for="active" required>Is Compulsory</label>
				<div class="col-8">
					<div class="form-check">
						<label class="form-check-label">
							<input class="form-check-input" type="checkbox" name="is_compulsory" id="is_compulsory"{is_compulsory} />
							<span class="form-check-sign"></span>
						</label>
					</div>
				</div>
		...
		<div class= "row">
			<div class="col-md-6">
				<button id='save_benefit_details' class="btn btn-primary">Save</button>
			</div>
			<div class="col-md-6 text-right">
				<button class="btn btn-outline-primary entity_back">Save & Continue</button>
			</div>
        ...
        javascript = """
          <script>
              setFormValidation('#form_benefit_details');
           $('#save_benefit_details').click(function(){
            var valid = FormIsValid("#form_benefit_details");
             if(valid){
                var formserial = $('#form_benefit_details').serialize();
                   $.post('/product/new_or_edit_benefit_details?', formserial, function(data){
                    var result = JSON.parse(data);
                    if(result.success === true){    
                    var kwargs =  'product_id='+result.product_id;
                
                     $('.nav-link:eq(1)').trigger('click');
                             $('#div_benefit_details').load('/product/get_edit_product_benefit_details_html?', kwargs, function(data){
                              return false;
                      });
                    };
                    return false;
        ...
        return  html + javascript
    @expose()
    def new_or_edit_benefit_details(self, *args, **kwargs):
        print(kwargs)
        benefit_id= kwargs.get('benefit_id')
        usernow = request.identity['user']
        this = Benefit.by_id(benefit_id)
        if not this: return ''
        this.name =kwargs.get('name')
        this.benefit_purchase_type_id = kwargs.get('benefit_purchase_type_id')
        this.is_compulsory = True if kwargs.get('is_compulsory') and kwargs.get('is_compulsory') == 'on' else False
        this.is_life = True if kwargs.get('is_life') and kwargs.get('is_life') == 'on' else False
        this.entity_org_insurer_id = kwargs.get('insurer_id')
    ...
    def get_edit_product_message_html(self, *args, **kwargs):
        html = f"""
                Message
             """
        return html

    def get_edit_product_documents_html(self, *args, **kwargs):
        html = f"""
                Documents
             """
        return html

    def get_edit_product_summary_html(self, *args, **kwargs):
        html = f"""
            Summary
             """
        return html
    ...
    @expose()
    def get_javascript_new_product_onload(self, *args, **kwargs):
        javascript = """
    ...
    def save_new_product(self, *args, **kwargs):
        ...
        contractual = TYPEUTIL.get_id_of_name('product_type', 'contractual')
        short_term_services = TYPEUTIL.get_id_of_name('product_type', "short_term")
        short_term_assets = TYPEUTIL.get_id_of_name('product_type', "short_term")

        redirect_url = '/product/index'
        if int(product_type_id) == voucher: redirect_url = '/product/edit_voucher'
        if int(product_type_id) == contractual: redirect_url = '/product/edit_traditional'
...
# ********************* Product Voucher & Traditional ***********************************#
    def get_selectbox_purchase(self, selected=None, *args, **kwargs):
    ...
    def get_selectbox_benefit_assured(_typeself, selected=None, *args, **kwargs):
        kwargs['id'] = 'benefit_asset_type_id'
        kwargs['selected'] = selected
        kwargs['outputdict'] = TYPEUTIL.get_dict_of_types("benefit_asset_type")
        return create_selectbox_html(**kwargs)

    def get_selectbox_benefit_assured_life_relationship(_typeself, selected=None, *args, **kwargs):
        kwargs['id'] = 'benefit_assured_life_relationship_type_id'
        kwargs['selected'] = selected
        kwargs['outputdict'] = TYPEUTIL.get_dict_of_types("benefit_assured_life_relationship_type")
        return create_selectbox_html(**kwargs)

    def get_selectbox_benefit_asset_type(_typeself, selected=None, *args, **kwargs):
        kwargs['id'] = 'benefit_asset_type_id'
        kwargs['selected'] = selected
        kwargs['outputdict'] = TYPEUTIL.get_dict_of_types("benefit_asset_type")
        return create_selectbox_html(**kwargs)
    ...
    def get_selectbox_product_owner(self, *args, **kwargs):
        kwargs['required'] = True
        kwargs['id'] = kwargs.get('id', 'entity_org_product_owner_id')
        outputlist = []
    ...
    def get_product_contract_documents_html(self, product=None, *args, **kwargs):
        ...
        if product.product_type_id == contractual:
